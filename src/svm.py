...
def runLinearSVM(data, task='Label', pickledModel=None, featureSelection = True, toPickle = False):
    ...
    if pickledModel is None:
        ...
        model = LinearSVC(loss='squared_hinge', dual=True, max_iter=5000)
    ...
    if task == 'Label':
        if pickledModel is None:
            ...
            if toPickle:
                pickle.dump(pipeline, open('SVC_Label_Model.pkl', 'wb'))
        ...
        if pickledModel is None:
            ...
            if toPickle:
                pickle.dump(pipeline, open('SVC_Attack_Model.pkl', 'wb'))
